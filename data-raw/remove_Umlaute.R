...
for (umlaut in 1:3) {
    ...
    agencies <- unique(df.gauging_station_data$agency[
        grepl(orig[umlaut], df.gauging_station_data$agency)])
    
    for (a in agencies) {
        print(a)
        b <- gsub(orig[umlaut], repl[umlaut], a)
        DBI::dbExecute(con,
                       paste0("UPDATE gauging_station_data SET agency = \'",
                              b, "\' WHERE agency = \'", a, "\'"))
    }
}
