...
The minimal required information to initialize a
...
rivers Elbe and Rhine and a determination of the time of the water level
...
visualize the results (`gauging_station`) or have an informative character (
...
water levels [@bundesanstalt_fur_gewasserkunde_flys_2016] with local gauging
...
contains daily-averaged gauging data since 1990-01-01. Therefore
`r lrd("waterLevel()")` can be applied for the time period  between 1990-01-01
and yesterday.
...
```{r figure1, fig.show = 'asis', fig.cap = capFig("Interpolated water level, computation-relevant stationary [FLYS3](http://www.bafg.de/DE/08_Ref/M2/03_Fliessgewmod/01_FLYS/flys_node.html) water levels (**0.5MQ**, **a** and **0.75MQ**) and gauge heigth of 2016-12-21 at River Elbe between Rosslau and Dessau."), eval = TRUE}
...
package-internal data, but online data provided by <https://pegelonline.wsv.de>
[@wsv_pegelonline_2018]. Because data provided by 
[PEGELONLINE](https://pegelonline.wsv.de) are available
for only 30 days backwards, the application of this function is limited to
recent time periods, but with a high temporal resolution.

```{r figure2, fig.show = 'asis', fig.cap = capFig(paste0("Interpolated water level, computation-relevant stationary [FLYS3](http://www.bafg.de/DE/08_Ref/M2/03_Fliessgewmod/01_FLYS/flys_node.html) water levels (**0.5MQ**, **a** and **0.75MQ**) and gauge heigth of ", strftime(Sys.time() - 3600, format = "%Y-%m-%d %H:%M")," at River Elbe between Rosslau and Dessau.")), eval = TRUE}
# one hour ago
...
```{r figure4, fig.show = 'asis', fig.cap = capFig("Water levels after [FLYS3](http://www.bafg.de/DE/08_Ref/M2/03_Fliessgewmod/01_FLYS/flys_node.html) with the reference gauge Rosslau at the 2016-12-21 at River Elbe between Rosslau and Dessau."), eval = TRUE}
...
modifying the dataset contents. This functionality is needed for **all**
`r lrd("waterLevel...()", "index.html#section-waterlevel-functions")`-functions
