...
lrd <- function(x, y) {
    # standard url
    url <- "http://r.bafg.de/~WeberA/hyd1d"
...
die aktuelle Entwicklungsversion vom Gitlab-Server der BfG installiert 
werden:

```{r install_git, eval = FALSE}
install.packages("devtools")
library(devtools)
devtools::install_git("git://gitlab.lan.bafg.de/auenoekologie/hyd1d")
```
...
[INFORM](http://www.bafg.de/DE/08_Ref/U2/02_analyse/01_INFORM/inform.html) 
...
in den beiden Datensätzen `r lrd("df.gauging_data")` und
`r lrd("df.gauging_station_data")` enthalten, können aber auch über eine
ShinyApp im Browser abgefragt, visualisiert und exportiert werden:
...
```{r figure19, echo = FALSE, fig.cap = capFig(paste0("Screenshot der ", href("gauging_data-ShinyApp", "http://r.bafg.de/shiny/WeberA/02-gauging_data/"), " mit Pegelstammdaten, Karte und Wasserstandszeitreihe für den Zeitraum vom 12.04.2017 bis 12.04.2018 am Pegel Rosslau.")), fig.show = 'asis', out.width = "100%",results = 'asis'}
...
```{r figure21, echo = FALSE, fig.cap = capFig(paste0("Screenshot der ", href("waterLevelPegelonline-ShinyApp", "http://r.bafg.de/shiny/WeberA/06-waterlevelpegelonline/"), " mit der interpolierten Wasserspiegellage und den berechnungsrelevanten stationären ", href("FLYS3", "http://www.bafg.de/DE/08_Ref/M2/03_Fliessgewmod/01_FLYS/flys_node.html"), "-Wasserspiegellagen (", bf("a"), ", ", bf("0.75MQ"), " und ", bf("b"), ") und Wasserständen vom 13.04.2018 11:00 Uhr an der Elbe bei Rosslau und Dessau.")), fig.show = 'asis', out.width = "100%"}
