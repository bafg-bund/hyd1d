...
waterLevelPegelonline <- function(wldf, shiny = FALSE){
    ...
    for (s in 1:(nrow(df.gs)-1)){
        ...
        if (df.gs$gauging_station[s] ==
            ...
            df.gs$n_wls_below_w_do[s + 1] <- sum(df.flys$w <= df.gs$wl[s])
            df.gs$n_wls_above_w_do[s + 1] <- sum(df.flys$w > df.gs$wl[s])
            ...
            #wldf_sub <- subset(wldf, station >= km_start & station <= km_end)
